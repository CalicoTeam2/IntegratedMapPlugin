package calico.plugins.IntegratedMapPlugin;
...
import calico.plugins.IntegratedMapPlugin.components.buttons.CreateIntegratedMapButton;
import calico.plugins.IntegratedMapPlugin.controllers.IntegratedMapController;
import calico.plugins.IntegratedMapPlugin.iconsets.CalicoIconManager;

public class IntegratedMapPlugin extends CalicoPlugin
	implements CalicoEventListener
{
	//the logger for this plugin
	public static Logger logger = Logger.getLogger(IntegratedMapPlugin.class.getName());
	
	public IntegratedMapPlugin()
	{
		super();
		PluginInfo.name = "ExamplePlugin";
		calico.plugins.IntegratedMapPlugin.iconsets.CalicoIconManager.setIconTheme(this.getClass(), CalicoOptions.core.icontheme);
	}
	...
	public void onPluginStart()
	{
		...
		CanvasStatusBar.addMenuButtonRightAligned(CreateIntegratedMapButton.class);
	...
	{
		return IntegratedMapPluginNetworkCommands.class;
	}
	...
	public void handleCalicoEvent(int event, CalicoPacket p) {
		switch (event) {
			case IntegratedMapPluginNetworkCommands.CUSTOM_SCRAP_CREATE:
...
//		Networking.send(p);	
	...
	private void CUSTOM_SCRAP_CREATE(CalicoPacket p) {
		...
		IntegratedMapController.create_custom_scrap(new_uuid, cuuid, x, y);
